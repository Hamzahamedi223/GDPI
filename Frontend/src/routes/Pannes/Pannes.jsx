...
const Pannes = () => {
  ...
  const handleExportPDF = () => {
    exportTableToPDF({
      ...
      filename: 'pannes',
      columns: ['Type', 'Équipement', 'Département', 'Description', 'Statut', 'Signalé par'],
      rows: filteredPannes.map(panne => [
        panne.type?.name || '',
        panne.equipment?.name || '',
        panne.department?.name || '',
        panne.description,
        panne.status,
        panne.reportedBy?.username || '',
      ]),
    });
  };
