...
import { Plus, AlertCircle, Truck, Edit2, Trash2, Mail, Phone, MapPin, Building, FileText, DollarSign, Eye, FileDown } from "lucide-react";
...
const Fournisseur = () => {
  ...
  const handleExportPDF = () => {
    exportTableToPDF({
      ...
      columns: ["Nom", "Contact", "Email", "Téléphone", "Adresse", "Statut"],
      rows: fournisseurs.map((f) => [
        f.name,
        f.contactPerson,
        f.email,
        f.phone,
        `${f.address?.street || ''}, ${f.address?.city || ''} ${f.address?.postalCode || ''}, ${f.address?.country || ''}`,
        f.status,
      ]),
    });
  };
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <motion.button
            ...
            className="flex items-center gap-x-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            <FileDown size={18} />
