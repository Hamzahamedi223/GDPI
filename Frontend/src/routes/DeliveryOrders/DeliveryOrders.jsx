...
const DeliveryOrders = () => {
            ...
            <tbody>
              ${order.items.map(item => `
                <tr>
                  ...
                  <td style="text-align: right;">${parseFloat(item.unitPrice).toFixed(2)} TND</td>
                  <td style="text-align: right;">${(parseFloat(item.quantity) * parseFloat(item.unitPrice)).toFixed(2)} TND</td>
                </tr>
              `).join('')}
            </tbody>
            <tfoot>
              <tr>
                ...
                <td style="text-align: right; font-weight: bold;">${calculateOrderTotal(order.items).toFixed(2)} TND</td>
              </tr>
            </tfoot>
          </table>
  ...
  const handleExportPDF = () => {
    exportTableToPDF({
      ...
      filename: "bons_de_livraison",
      columns: ["Référence", "Client", "Date", "Total", "Statut"],
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-semibold mt-2 text-gray-900 dark:text-white">
                  {formatCurrency(deliveryOrders.reduce((total, order) => total + (order.total || 0), 0))}
                </p>
              </div>
            ...
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="ml-2 flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 dark:bg-slate-700 dark:text-gray-200 dark:hover:bg-slate-600"
                >
        ...
        <div>
          ...
          <div className="mt-6 flex justify-end">
            <div className="text-right">
              <p className="text-sm text-gray-500 dark:text-gray-400">Total</p>
              <p className="text-2xl font-semibold text-gray-900 dark:text-white">
                {formatCurrency(calculateTotal(data.items))}
              </p>
            </div>
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tfoot>
                <tr>
                  ...
                  <td style={{ textAlign: "right", fontWeight: "bold" }}>{formatCurrency(calculateTotal(order.items))}</td>
                </tr>
              </tfoot>
            </table>
          </div>
