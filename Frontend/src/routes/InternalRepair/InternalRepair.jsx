...
import { Plus, AlertCircle, Tag, Calendar, CheckCircle, XCircle, Edit2, Trash2, Clock, FileDown } from 'lucide-react';
...
const InternalRepair = () => {
  ...
  const handleExportPDF = () => {
    exportTableToPDF({
      title: 'Liste des Réparations Internes',
      filename: 'reparations_internes',
      columns: ['Équipement', 'Pièce de rechange', 'Description', 'Date ajout', 'Date réparation', 'Statut'],
      rows: repairs.map(repair => [
        `${repair.equipment?.name || ''} (SN:${repair.equipment?.serial_number || ''})`,
        repair.sparePart?.name || '',
        repair.description,
        repair.dateAdded ? new Date(repair.dateAdded).toLocaleDateString() : '',
        repair.repairDate ? new Date(repair.repairDate).toLocaleDateString() : '',
        repair.status,
      ]),
    });
  };
